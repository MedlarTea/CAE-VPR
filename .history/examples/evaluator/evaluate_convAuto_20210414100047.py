...
def drawL2Distribution(root_dir, method, dataset):
    ...
    plt.figure()
    counts_x, bins_x = np.histogram(trueMatches,bins=25)
    plt.hist(bins_x[:-1], bins_x, weights=counts_x/len(trueMatches), color="darkorange", edgecolor = 'black', alpha=0.6, label="True matches(u={:.3f}, var={:.3f})".format(true_mean, true_std))
    ...
    plt.axvline(x=true_mean, linestyle='--', color='magenta')
    counts_y, bins_y = np.histogram(falseMatches,bins=25)
    ...
    plt.hist(bins_y[:-1], bins_y, weights=counts_y/len(falseMatches), color="darkcyan", edgecolor = 'black', alpha=0.6, label="False matches(u={:.3f}, var={:.3f})".format(false_mean, false_std))
    plt.axvline(x=false_mean, linestyle='--' , color='magenta')
    plt.legend()
