...
def drawL2Distribution(root_dir, method, dataset):
    ...
    plt.hist(bins_x[:-1], bins_x, weights=counts_x/len(trueMatches), color="red", alpha=0.8, label="True matches(u={:.3f}, var={:.3f})".format(true_mean, true_std))

    plt.axvline(x=true_mean, linestyle='--')
    counts_y, bins_y = np.histogram(falseMatches,bins=25)
    plt.hist(bins_y[:-1], bins_y, weights=counts_y/len(falseMatches), color="green", alpha=0.8, label="False matches(u={:.3f}, var={:.3f})".format(false_mean, false_std))
