...
def visualize(infeatures, image_path, returnlayer="conv5", visType="pow2", height=480, width=640):
    ...
    for index, infeature in enumerate(infeatures):
        ...
        # store path
        basename = osp.basename(image_path).strip('.jpg') + "_{}_{}.jpg".format(returnlayer, visType)
        store_path = osp.join(osp.dirname(image_path), basename)
        im.save(store_path)
...
if __name__ == '__main__':
    ...
    # Load image and transform it
    image_path = "/home/lab/data1/hanjing/singleframe/BerlinA100_RAS2020/Live/image0017.png"
    img = get_image(image_path)

    # extract descriptor (4096-dim)
    with torch.no_grad():
        returnlayers, _ = model(img.unsqueeze(0))
    
    # visualize
    for name, feature in returnlayers:
        visualize(feature, image_path, returnlayer=name, visType="pow2", height=480, width=640)
