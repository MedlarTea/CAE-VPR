...
def drawL2Distribution(root_dir, method, dataset):
    ...
    counts_x, bins_x = np.histogram(trueMatches)
    ...
    plt.hist(bins_x[:-1], bins_x, weights=counts_x, color="darkorange", edgecolor = 'black', alpha=0.5, label="True matches(u={:.3f}, var={:.3f})".format(true_mean, true_std))
    plt.axvline(x=true_mean)
    # counts_y, bins_y = np.histogram(falseMatches,bins=15)
    counts_y, bins_y = np.histogram(falseMatches)
    ...
    plt.hist(bins_y[:-1], bins_y, weights=counts_y, color="darkcyan", edgecolor = 'black', alpha=0.5, label="False matches(u={:.3f}, var={:.3f})".format(false_mean, false_std))
    plt.axvline(x=false_mean)
    plt.legend()
