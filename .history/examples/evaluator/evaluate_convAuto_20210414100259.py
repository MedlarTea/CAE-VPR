...
def drawL2Distribution(root_dir, method, dataset):
    ...
    plt.hist(bins_x[:-1], bins_x, weights=counts_x/len(trueMatches), color="darkorange", alpha=0.6, label="True matches(u={:.3f}, var={:.3f})".format(true_mean, true_std))
    ...
    plt.hist(bins_y[:-1], bins_y, weights=counts_y/len(falseMatches), color="darkcyan", alpha=0.6, label="False matches(u={:.3f}, var={:.3f})".format(false_mean, false_std))
