...
def remove_dynamic(semantic_path, image, _max):
    semantic_mask = pil.open(semantic_path)
    se = np.array(semantic_mask, dtype=np.int)
    _max = max(_max, se.max())
    # print("se min:{}, max:{}".format(se.min(), se.max()))
    ...
    # print("semantics min:{}, max:{}".format(semantic_mask.min(), semantic_mask.max()))

    semantic_mask[semantic_mask > 9] = 99
    semantic_mask[semantic_mask != 99] = 1
    semantic_mask[semantic_mask == 99] = 0
    filtered_image = torch.mul(semantic_mask, image)
    return filtered_image, _max

def testRemoveDynamic():
    model = get_model()
    ...
    # query
    _max = 0
    query_dir = "/home/lab/data1/hanjing/singleframe/Kudamm_RAS2020/Live"
    query_semantic_dir = "/home/lab/data1/hanjing/singleframe/Kudamm_RAS2020/Live_semantics"
    images = os.listdir(query_dir)
    _max = 0
    q_descs = []
    for index, image in enumerate(images):
        img = get_image(os.path.join(query_dir, image))
        img_no_dynamic, _max = remove_dynamic(os.path.join(query_semantic_dir, image), img, _max)
        with torch.no_grad():
            _, q_masked_desc = model(img_no_dynamic.unsqueeze(0))
        q_descs.append(q_masked_desc.squeeze().cpu().numpy())
        print("{}:{}".format(index, len(images)))
    
    db_dir = "/home/lab/data1/hanjing/singleframe/Kudamm_RAS2020/Reference"
    db_semantic_dir = "/home/lab/data1/hanjing/singleframe/Kudamm_RAS2020/Reference_semantics"
    images = os.listdir(db_dir)
    db_descs = []
    for index, image in enumerate(images):
        img = get_image(os.path.join(db_dir, image))
        img_no_dynamic, _max = remove_dynamic(os.path.join(db_semantic_dir, image), img, _max)
        with torch.no_grad():
            _, db_masked_desc = model(img_no_dynamic.unsqueeze(0))
        db_descs.append(db_masked_desc.squeeze().cpu().numpy())
        print("{}:{}".format(index, len(images)))

    print("max: {}".format(_max))
    distance = pairwise_distances(q_descs, db_descs)
    args = np.argsort(distance, axis=1)
    args = args[:,:5]
    np.savetxt("./results/Kudamm_RAS2020_rmDynamics.txt", args)
...
if __name__ == '__main__':
    ...
    # ckpt_path = "/home/lab/data1/hanjingModel/OpenIBL_forRobotcar/logs/netVLAD/attention/teacher/robotcar-vgg16/conv5-triplet-lr0.001-neg10-tuple2/model_best.pth.tar"
    # ckpt_path = "/home/lab/data1/hanjingModel/OpenIBL_forRobotcar/logs/netVLAD/attention/student/robotcar30k-vgg16/conv5-conv6-vistypepow2-vprLtriplet-attLmse-lvpr1.0-latt1.0-lr0.001-neg10-tuple2/checkpoint20.pth.tar"
    ckpt_path = "/home/lab/data1/hanjingModel/OpenIBL_forRobotcar/logs/netVLAD/attention/student/robotcar30k-vgg16/conv5-conv6-vistypepow2-vprLtriplet-attLmse-lvpr1.0-latt1.0-lr0.001-neg10-tuple2/model_best.pth.tar"

    load_ckpt(model, ckpt_path)

    # Visualize the Att map
    # Load image and transform it
    image_path = "./testImages/image0158.jpg"
    result_path = "./testImages/image0158.jpg"
