...
def remove_dynamic(semantic_path, image):
    semantic_mask = pil.open(semantic_path)
    semantic_mask = np.array(semantic_mask, dtype=np.int)
    print("se min:{}, max:{}".format(semantic_mask.min(), semantic_mask.max()))
    transformer = transforms.Compose([transforms.Resize((480,640)), transforms.ToTensor()])
    semantic_mask = transformer(semantic_mask)
...
def testRemoveDynamic():
    ...
    for index, image in enumerate(images):
        ...
        img_no_dynamic = remove_dynamic(os.path.join(query_semantic_dir, image), img)
        with torch.no_grad():
    ...
    for index, image in enumerate(images):
        ...
        img_no_dynamic = remove_dynamic(os.path.join(db_semantic_dir, image), img)
        with torch.no_grad():
    ...
    np.save("./results/Kudamm_RAS2020_rmDynamics.txt", args)
