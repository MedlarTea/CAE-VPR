...
from ibl.trainers import AttentionTrainer
...
def get_data(args, iters):
    ...
    train_loader = IterLoader(
                DataLoader(Preprocessor(dataset.q_train+dataset.db_train, root=dataset.images_dir,
                                        transform=train_transformer, useSemantics=args.useSemantics, isRobotcar=True if args.dataset=="robotcar" else False),
    ...
    train_extract_loader = DataLoader(
        Preprocessor(sorted(list(set(dataset.q_train) | set(dataset.db_train))),
                     root=dataset.images_dir, transform=test_transformer, isRobotcar=True if args.dataset=="robotcar" else False),
    ...
    val_datasets["BerlinA100_RAS2020"] = datasets.create("singleframe", root, city="BerlinA100_RAS2020")
    val_datasets["Halen_RAS2020"] = datasets.create("singleframe", root, city="Halen_RAS2020")
    val_datasets["Kudamm_RAS2020"] = datasets.create("singleframe", root, city="Kudamm_RAS2020", isjpg=True)
    val_datasets["Gardens_point_RAS2020"] = datasets.create("singleframe", root, city="Gardens_point_RAS2020", isjpg=True)
    for name in val_datasets.keys():
        val_loader_q = DataLoader(
            Preprocessor(val_datasets[name].q_test, root=val_datasets[name].images_dir, transform=test_transformer, isRobotcar=True if name.find("robotcar")!=-1 else False),
            batch_size=(1 if args.dataset=='tokyo' else args.test_batch_size), num_workers=args.workers,
            sampler=DistributedSliceSampler(val_datasets[name].q_test),
            shuffle=False, pin_memory=True)
        val_loader_qs[name] = val_loader_q

        val_loader_db = DataLoader(
            Preprocessor(val_datasets[name].db_test, root=val_datasets[name].images_dir, transform=test_transformer, isRobotcar=True if name.find("robotcar")!=-1 else False),
            batch_size=args.test_batch_size, num_workers=args.workers,
            sampler=DistributedSliceSampler(val_datasets[name].db_test),
            shuffle=False, pin_memory=True)
        val_loader_dbs[name] = val_loader_db


    return dataset, train_loader, val_loader, test_loader, sampler, train_extract_loader, val_datasets, val_loader_qs, val_loader_dbs
...
def update_sampler(sampler, model, loader, query, gallery, sub_set, vlad=True, gpu=None, sync_gather=False):
    ...
    distmat, _, _ = pairwise_distance(features, query, gallery, isRobotcar=True)
    del features
    if (dist.get_rank()==0):
...
def get_model(args):
    if args.vlad:
        base_model = models.create("vgg16", cut_at_pooling=False, train_layers=args.layers, matconvnet='logs/vd16_offtheshelf_conv5_3_max.pth', pretrained=True, return_layer=args.return_layer)
        ...
        model = models.create('embedattentionNet', base_model, pool_layer, useSemantics=args.useSemantics, visType=args.visType)
    else:
        base_model = models.create("vgg16", cut_at_pooling=False, train_layers=args.layers, matconvnet='logs/vd16_offtheshelf_conv5_3_max.pth', pretrained=True, return_layer=None)
        model = base_model
...
def main_worker(args):
    ...
    dataset, train_loader, val_loader, test_loader, sampler, train_extract_loader, val_datasets, val_loader_qs, val_loader_dbs = get_data(args, iters)

    # Create model
    model = get_model(args)
    
    # Load from teacher checkpoint
    # if args.teacher_resume:
    #    checkpoint = load_checkpoint(args.teacher_resume)
    ...
    auc, recalls = evaluator.evaluate(val_loader, sorted(list(set(dataset.q_val) | set(dataset.db_val))),
                        ...
                        vlad=args.vlad, gpu=args.gpu, sync_gather=args.sync_gather, isRobotcar = True)
    for name in val_datasets.keys():
        if (args.rank==0):
            print("==========\nDataset:{}\n==========".format(name))
        s_auc, s_recalls = evaluator.evaluate(val_loader_qs[name], sorted(list(set(val_datasets[name].q_test) | set(val_datasets[name].db_test))),
                        val_datasets[name].q_test, val_datasets[name].db_test, val_datasets[name].test_pos,
                        vlad=args.vlad, gpu=args.gpu, sync_gather=args.sync_gather, isRobotcar = True if name.find("robotcar")!=-1 else False)
    ...
    # Trainer
    trainer = AttentionTrainer(model, margin=args.margin**0.5, gpu=args.gpu, useSemantics=args.useSemantics, temp=0.07, lambda_vpr=args.lambda_vpr, lambda_att=args.lambda_att)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for subid, subset in enumerate(subset_indices):
            ...
            trainer.train(epoch, subid, train_loader, optimizer,
                            ...
                            vlad=args.vlad, vpr_loss_type=args.vpr_loss_type, att_loss_type=args.att_loss_type)
            synchronize()
        ...
        if ((epoch+1)%args.eval_step==0 or (epoch==args.epochs-1)):
            ...
            for name in val_datasets.keys():
                if (args.rank==0):
                    print("==========\nDataset:{}\n==========".format(name))
                    print("  test_query    | {:8d}"
                        .format(len(val_datasets[name].q_test)))
                    print("  test_gallery  | {:8d}"
                        .format(len(val_datasets[name].db_test)))
                s_auc, s_recalls = evaluator.evaluate(val_loader_qs[name], sorted(list(set(val_datasets[name].q_test) | set(val_datasets[name].db_test))),
                            val_datasets[name].q_test, val_datasets[name].db_test, val_datasets[name].test_pos, gallery_loader=val_loader_dbs[name],
                            ...
                            rr_topk=args.rr_topk, lambda_value=args.lambda_value, pr_image_path=None,
                            distri_curve_path=None, matching_examples_path=None, return_layer=args.return_layer, isRobotcar = True if name.find("robotcar")!=-1 else False)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="NetVLAD/NetVLAD_Att training")
    ...
    parser.add_argument('--useSemantics', action='store_true', help='是否使用语义图')
    parser.add_argument('--visType', type=str, default="abs_sum", choices=['abs_sum', 'max', 'pow2'])
    parser.add_argument('--return_layer', type=str, default="conv6", choices=[None, 'conv6', 'conv5', 'conv4', "conv3", "conv2"])
    # optimizer
    parser.add_argument('--lr', type=float, default=0.001,
                        help="learning rate of new parameters, for pretrained ")
    parser.add_argument('--momentum', type=float, default=0.9)
    parser.add_argument('--weight-decay', type=float, default=0.001)
    parser.add_argument('--vpr_loss_type', type=str, default='triplet', help="[triplet|sare_ind|sare_joint]")
    parser.add_argument('--att_loss_type', type=str, default='mse', help="[mse|l1]")
    parser.add_argument('--step-size', type=int, default=5)
    parser.add_argument('--lambda_vpr', type=float, default=1.0)
    parser.add_argument('--lambda_att', type=float, default=1.0)
    # training configs
    parser.add_argument('--resume', type=str, default='', metavar='PATH')
    parser.add_argument('--teacher_resume', type=str, default='', metavar='PATH')
