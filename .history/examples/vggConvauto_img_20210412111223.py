...
def get_data(args, iters):
    ...
    train_loader = DataLoader(Preprocessor(dataset.q_train+dataset.db_train, root=root, transform=train_transformer),
                            batch_size=args.bs, num_workers=args.workers, sampler=sampler,
                            shuffle=False, pin_memory=True)
...
def get_model(args):
    if args.arch == 'vgg':
        ...
        base_model = models.create("alexnet", cut_layer="conv5", matconvnet='logs/vd16_offtheshelf_conv5_3_max.pth', isForConvautoTrain=True)
...
def main_worker(args):
    ...
    auc, recalls = evaluator.evaluate(val_loader, sorted(list(set(dataset.q_val) | set(dataset.db_val))),
                        ...
                        vlad=False, gpu=args.gpu, sync_gather=args.sync_gather)
