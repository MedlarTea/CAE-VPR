...
def get_data(args, iters):
    ...
    train_loader = IterLoader(
                DataLoader(Preprocessor(dataset.q_train+dataset.db_train, root=dataset.images_dir,
                                        transform=train_transformer, isRobotcar=True if args.dataset=="robotcar" else False),
...
def get_model(args):
    base_model = models.create(args.arch, train_layers=args.layers, matconvnet='logs/vd16_offtheshelf_conv5_3_max.pth', pretrained=True)
    if args.vlad:
        pool_layer = models.create('netvlad', dim=base_model.feature_dim)
        # vgg16_pitts_64_desc_cen_mat.hdf5
        initcache = osp.join(args.init_dir, args.arch + '_' + 'pitts' + '_' + str(args.num_clusters) + '_desc_cen.hdf5')
        if (dist.get_rank()==0):
        ...
        semantic_layer = models.create('semanticnet', input_channel=1, output_channel=64)  # semantic
        model = models.create('embednet', base_model, pool_layer, semantic_layer)
    else:
        model = base_model
