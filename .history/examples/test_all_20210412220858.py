...
def get_data(args):
    ...
    allDatasets["robotcar_qAutumn_dbSunCloud"] = datasets.create("robotcar", root, datelist=osp.join(root, 'datelist_for_PR.txt'), q_date='2014-12-09-13-21-02', db_val_date='2014-12-16-18-44-24', db_test_date='2014-11-18-13-20-12')
    allDatasets["robotcar_qAutumn_dbNight"] = datasets.create("robotcar", root, datelist=osp.join(root, 'datelist_for_PR.txt'), q_date='2014-12-09-13-21-02', db_val_date='2014-11-18-13-20-12', db_test_date='2014-12-16-18-44-24')
    allDatasets["robotcar_qSnow_dbNight"] = datasets.create("robotcar", root, datelist=osp.join(root, 'datelist_for_PR.txt'), q_date='2015-02-03-08-45-10', db_val_date='2014-11-18-13-20-12', db_test_date='2014-12-16-18-44-24')
    allDatasets["robotcar_qSnow_dbSunCloud"] = datasets.create("robotcar", root, datelist=osp.join(root, 'datelist_for_PR.txt'), q_date='2015-02-03-08-45-10', db_val_date='2014-12-16-18-44-24', db_test_date='2014-11-18-13-20-12')
    ...
    allDatasets["Berlin_Halenseetrasse_RAS2020"] = datasets.create("singleframe", root, city="Berlin_Halenseetrasse_RAS2020")
    allDatasets["Berlin_Kudamm_RAS2020"] = datasets.create("singleframe", root, city="Berlin_Kudamm_RAS2020", isjpg=True)
    ...
    for name in allDatasets.keys():
        test_loader_q = DataLoader(
            Preprocessor(allDatasets[name].q_test, root=allDatasets[name].images_dir, transform=test_transformer_q),
        ...
        test_loader_db = DataLoader(
            Preprocessor(allDatasets[name].db_test, root=allDatasets[name].images_dir, transform=test_transformer_db),
            batch_size=args.test_batch_size, num_workers=args.workers,
...
def main_worker(args):
    ...
    for name in allDatasets.keys():
        if (args.rank==0):
            log_dir = osp.dirname(args.resume)
            result_path = osp.join(log_dir, "log_test_{}.txt".format(name))
            pResult = open(result_path, "w")
        if (len(allDatasets[name].q_test) > 4500):
            continue
        if (args.rank==0):
            print("==========\nDataset:{}\n==========".format(name))
        ...
        auc, recalls = evaluator.evaluate(test_loader_qs[name], sorted(list(set(allDatasets[name].q_test) | set(allDatasets[name].db_test))),
                            ...
                            distri_curve_path=distri_curve_path, matching_examples_path=matching_examples_path, return_layer=args.return_layer)
        if (args.rank==0):
