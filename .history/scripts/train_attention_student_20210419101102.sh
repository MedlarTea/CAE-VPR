...
EPOCHS=30
TUPLESIZE=1
TRESUME=$3  # logs/xxx/xxxx.tar.gz
MARGIN=0.1
...
CUDA_VISIBLE_DEVICES=2,3 $PYTHON -m torch.distributed.launch --nproc_per_node=$GPUS --master_port=$PORT --use_env \
...
--margin ${MARGIN} --eval-step 1 --epochs ${EPOCHS} --step-size 5  --teacher_resume ${TRESUME} \
--logs-dir logs/netVLAD/attention/student/${DATASET}${SCALE}-${ARCH}/${LAYERS}-${RETURNLAYER}-vistype${VISTYPE}-vprL${VPRLOSS}-attL${ATTLOSS}-lvpr${LVPR}-latt${LATT}-lr${LR}-neg${NEGNUM}-tuple$[${TUPLESIZE}*${GPUS}] \
--data-dir ${DATRDIR}
