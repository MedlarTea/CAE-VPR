...
CUDA_VISIBLE_DEVICES=1,2,3 $PYTHON -m torch.distributed.launch --nproc_per_node=$GPUS --master_port=$PORT --use_env \
examples/test_all.py --launcher pytorch \
    -a ${ARCH} \
    --test-batch-size 30 -j 8 \
    --resume ${RESUME} \
    --data_dir ${DATADIR} \
    --dimension ${DIMENSION} \
    --return_layer ${RETURNLAYER} \
    --visType ${VISTYPE}\
    --reduction --vlad # in convAuto--vgg16 test, this must be cancled
    # --isvisualized 
    # --sync-gather
