...
def get_data(args):
    ...
    train_extract_loader = DataLoader(
        Preprocessor(pitts_train, root=dataset.images_dir, transform=test_transformer_db, isRobotcar=False),
...
def get_model(args):
    ...
    # for vgg16+NetVLAD, note:args.vlad==True, args.reduction=true
    elif args.arch == "netvlad":
        args.vlad = True
        args.reduction = True
...
def main_worker(args):
    ...
    if args.resume and args.arch!='alexnet':
    ...
    if (args.reduction):
        pca_parameters_path = osp.join(osp.dirname(args.resume), 'pca_params_'+str(args.features)+'_'+osp.basename(args.resume).split('.')[0]+'.h5')
    ...
    for name in allDatasets.keys():
        ...
        if (args.rank==0):
            pResult.write("Auc Recall@1 Recall@5 Recall@10 Recall@20: %0.3f\n"%auc)
            pResult.write("&%0.3f &%0.3f &%0.3f &%0.3f &%0.3f\n"%(auc, recalls[0], recalls[1], recalls[2], recalls[3]))
    if (args.rank==0):
        pResult.close()
        # sys.stdout.__exit__()
    synchronize()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--arch', type=str, default='vgg16',
                        choices=["vggConv", "alexnetConv", "netvlad", "vgg16","alexnet"])
    ...
    parser.add_argument('--return_layer', type=str, default=None, choices=[None, 'conv6', 'conv5', 'conv4', "conv3", "conv2"])
    # path
