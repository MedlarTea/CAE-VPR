...
class DistributedRandomDiffTupleSampler(Sampler):
    ...
    def __iter__(self):
        ...
        for i in indices:
            ...
            # hardest negatives from a random image pool and previous epoch
            neg_candidates = [x for x in self.sort_idx[anchor_index].tolist() if x not in self.neg_list[anchor_index]]  # 获取负样本, 不在self.neg_list的即为负样本
            neg_pool_index = random.sample(range(len(neg_candidates)), min(self.neg_pool,len(neg_candidates)))  # 打乱负样本, 选1000个样本
            neg_cache_index = [neg_candidates.index(i) for i in self.neg_cache[anchor_index]]  # 来自上一次迭代的负样本
            neg_pool_index = sorted(list(set(neg_pool_index) | set(neg_cache_index)))  # 求并集，排序
            neg_indices = [neg_candidates[i] for i in neg_pool_index[:self.neg_num]]  # 选择10个负样本
            self.neg_cache[anchor_index] = neg_indices
            assert(len(neg_indices)==self.neg_num)

            iters = [anchor_index, pos_top1 + len(self.query_source)]  # query
            iters += [n + len(self.query_source) for n in neg_indices]  # db-负样本
            iters += [p+len(self.query_source) for p in pos_indices]  # db-正样本
            yield iters
