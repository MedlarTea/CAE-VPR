...
def extract_cnn_feature(model, inputs, vlad=True, gpu=None):
    ...
    inputs = to_torch(inputs) # for pca
    ...
    if (isinstance(outputs, list) or isinstance(outputs, tuple) and len(outputs)!=3):
        ...
        if vlad:
            ...
            # for evaluation of vgg16 and alexnet
...
def pairwise_distance(features, query=None, gallery=None, isRobotcar=False, metric=None):
...
class Evaluator(object):
    ...
    def evaluate(self, query_loader, dataset, query, gallery, ground_truth, gallery_loader=None, \
                    ...
                    matching_examples_path=None, return_layer="conv5", isRobotcar = False):
        ...
        distmat, _, _ = pairwise_distance(features, query, gallery, isRobotcar=isRobotcar)
        ...
        if (self.rank==0):
            ...
            distmat_gg, _, _ = pairwise_distance(features, gallery, gallery, isRobotcar=isRobotcar)
            distmat_qq, _, _ = pairwise_distance(features, query, query, isRobotcar=isRobotcar)
